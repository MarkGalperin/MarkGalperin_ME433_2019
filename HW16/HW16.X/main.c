    ...
    while(1) {
        ...
        */
        ...
        for (i=0;i<c/2;i++){
            if (bright[i]<avg){
                // count this pixel
                LCD_drawPixel(i,30,ILI9341_CYAN); // visualize which pixels we're counting
                sum = sum + 255;
                sumR = sumR + 255*i;
            }
            else {
        ...
        if (sum>10){
            com = sumR/sum; //com = average value of qualifying locations
        }
        else {
            com = c/2/2;
        }
        ...
        if (com < c/2/2){ //A HIGHER (turn LEFT)
            e = c/2/2 - com;
            speed = minspeed + kp*e;
            ...
            drawString(140,92,message);
            /*
            speed = minspeed + ((maxspeed-minspeed)/c/12)*e; // when the com is all the way over, the motor is all off
            if(speed > maxspeed){
                speed = maxspeed;
            }
            if(speed < 0){
